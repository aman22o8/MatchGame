...
import './App.css'
import MatchTab from './Component/MatchTab'
...
class App extends Component {
  ...
  tick = () => this.setState(prev => ({timersecond: prev.timersecond - 1}))
  ...
  getfiltereditems = () => {
    ...
    const filtereditemss = imagesList.filter(each => each.category === activeId)
    return filtereditemss
  }

  clearkaro = () => clearInterval(this.intervalId)

  randonfunctionindex = id => {
    const {initialImage} = this.state
    const randomindex = this.myrandom()
    this.setState({initialImage: imagesList[randomindex]})
    // console.log('random index', randomindex)
    const index = imagesList.find(each => each.id === id)
    console.log('index', index)
    console.log('random image', initialImage)
    if (initialImage.id === index.id) {
      console.log('you are right')
      this.setState(prev => ({countId: prev.countId + 1}))
    } else {
      this.setState({initialcondition: false})
      clearInterval(this.intervalId)
    }
  }

  myrandom = () => {
    const myvalue = Math.round(Math.random() * 30)
    // const myvalue = imagesList.sort(() => Math.random() - 0.5)
    return myvalue
  }
  ...
  render() {
    ...
    const mycondition = timersecond !== 0 && initialcondition === true
    return (
      <div className="bg_container">
        <div className="header_container">
          <img
            ...
            alt="website logo"
          />
          <div className="score_timer_container">
            <p className="score">Score:{countId}</p>
            <img
              className="timer_logo"
              src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png"
              alt="timer"
            />
            <p className="timer_heading">
              {timersecond > 9 ? timersecond : `0${timersecond}`} sec
            </p>
          </div>
        </div>
        <div className="centerkaro">
          {mycondition ? (
            <div className="footer_container">
              ...
              <ul className="tab_item">
                {filteredlist.map(each => (
                  <MatchItem
                    randomfunction={this.randonfunctionindex}
                    key={each.id}
                    displayitem={each}
                  />
                ))}
            ...
            <div className="scorecard_container">
              <img
                className="score_logo"
                src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
                alt="trophy"
              />
              <p className="score_heading">Your Score</p>
              <p className="score_heading">{countId}</p>
              <button
                onClick={this.playagainfunction}
                type="button"
